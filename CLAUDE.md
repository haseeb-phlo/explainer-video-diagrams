...
**Day 3 (`videos/ai-training/3-prompting-crispe-framework/`) is built from component scenes, not one long board.** Unlike the 9-beat `claude/` videos, this session is a set of small self-contained scenes, one `build*.py` + one `.excalidraw` each, all in the day's folder: `build_prompt_anatomy.py` → `prompt-anatomy.excalidraw` (the vague-vs-annotated prompt pair plus the six-dial CRISPE side panel) and `build_your_setup.py` → `your-setup.excalidraw` (the five-toggle settings dashboard plus the voice-capture phone). Each is a single composed beat sized to the standard slot, so the `GAP` between beat slots never comes into play; where the brief asks for something to be "revealed one by one", the reveal is baked in as numbered badges and the pan order is documented in the build script's docstring — there is no animation to author. **The framework is CRISPE** (Context, Role, Instructions, Style, Parameters, Example) — six letters, not the five-letter CRISP that older drafts of the curriculum mapping used.
