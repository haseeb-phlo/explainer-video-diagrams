...
2. **Eyeball it with the PIL preview.** There's no Excalidraw CLI, so the repo-root `preview.py` rasterises a scene to a PNG — approximate (flat fills, macOS hand-font stand-ins for the Virgil hand font) but enough to check composition, colour, overlap and text overflow without opening the app.
   ```bash
   python3 preview.py videos/Codex/3-artefacts/3-artifacts.excalidraw out.png            # whole board
   python3 preview.py videos/Codex/3-artefacts/3-artifacts.excalidraw out.png XMIN XMAX  # close-up x-window
   ```
...
The style is **one flowing, illustrated, hand-drawn journey** — modelled on `videos/ai-foundations/1-what-is-ai/llm_explainer.excalidraw` (the preferred reference). The canonical build is `videos/Codex/3-artefacts/`. Rules:
...
- **Charming primitive illustrations** (sketched Codex face, chat-left/Artifact-panel-right window, colour-coded object doodles) carry visual interest on hero beats; light rotation jitter on notes/chips. Prefer hand-drawn icons over emoji.
...
- **Live-demo cut-aways.** Where the presenter drops out to the live Codex desktop app, mark it on the board with a `demo_badge(x, y, "show in Codex desktop app: …")`, and use `[CUT TO Codex DESKTOP]` / `[BACK TO BOARD]` cues in the matching script. The flowing per-beat layout supports cutting away and returning to the same beat with no structural change.

Keep British English, hyphens not em-dashes, and fictional identifiers only. **Examples must be generic, not Phlo-specific** — no Phlo branding/logo, and no patient/clinical specifics. Use everyday business scenarios (a customer reply, a budget calculator, a team tone guide, Support / Ops) so a video is reusable. Product proper nouns stay exact: Codex, Artifact, Project, Skill, MCP, Connector, Cowork, Codex Desktop. (This supersedes SKILL.md's older "use *Patient*" vocabulary rule.)
...
.Codex/skills/SKILL.md       # the phlo-learn-videos skill (visual + safety conventions)
scripts/                      # session launchers + compile_excalimate.py
```

Series seen so far: `ai-foundations/` (Module 1 concepts), `Codex/` (Module 2 Codex walkthrough), and a stubbed `copilot/`. Each video folder also tends to hold a narration/voiceover script (`*.md` or `*-script`) and an `animation-plan.md` documenting the build so it can be rebuilt or re-exported by hand. As of 2026-06-04 **all twelve `Codex/` videos (1-12) and both built `ai-foundations/` videos have a Style-B `build*.py` + `.excalidraw`**; the only remaining **empty placeholder** is `ai-foundations/3-context-windows/` — an empty folder means "not started", not "lost work".

## Non-negotiable conventions (from `.Codex/skills/SKILL.md`)

Read `.Codex/skills/SKILL.md` in full before authoring a diagram. The hard rules:
...
- **Vocabulary is fixed:** "Phlo" not "we/us"; "Patient" not "user/customer"; "Codex"/"Granola"/"Beacon"/"MCP"/"Codex" are proper nouns with exact capitalisation.
...
- The skill lives at `.Codex/skills/SKILL.md` (skill name is still `phlo-learn-videos`) and work is organised under `videos/<series>/<NN-slug>/`. README and SKILL.md have both been brought in line with this and with the static/flowing workflow.
...
- **Ownership, to stop drift recurring:** `.Codex/skills/SKILL.md` owns the drawing rules, safety guardrails, and vocabulary; `README.md` owns the human production workflow; this file is the operating map and cross-references the other two rather than restating them. SKILL.md uses the lively non-brand Excalidraw palette deliberately — there is no "replace the brand hex" step.
- `scripts/compile_excalimate.py` (legacy) hardcodes `llm_explainer.*` paths relative to repo root.
- **All twelve Module 2 Codex videos are built** with Style B `build_*.py` scripts that live next to their video and write output via `os.path.dirname(__file__)` (lands in-folder): `1-Codex-intro/build_claude_interface.py` → `claude_intro.excalidraw`; `2-projects/build_projects.py` → `phlo-2.2-Codex-projects.excalidraw`; `3-artefacts/build_excalidraw.py`, then `build_skills_training.py`, `build_scheduled_tasks.py`, `build_connectors_mcp.py`, `build_powerpoint.py`, `build_excel.py`, `build_word.py`, `build_web_extension.py`, `build_cowork.py`, `build_design.py`. The Projects video was **converted from a rejected frame-based "Style A" deck** (10 fixed `1920x1080` frames, purple/lilac palette); that old generator (`build_projects_2_2.py`) has been deleted — do not reintroduce a frame-based build.
- `videos/Codex/1-Codex-intro/` has a single canonical scene, `claude_intro.excalidraw`, generated by `build_claude_interface.py`. (The script's docstring + `claude_intro_prompt.md` still say `Codex-interface-properly.excalidraw`; the live `out =` target is `claude_intro.excalidraw` — trust the code. Regenerate from the script; don't keep a second hand-named copy.)
- Several `*_prompt`/`*_prompt.md` files (e.g. `1-Codex-intro/claude_intro_prompt.md`) still describe the **old editorial Style A** (fontFamily 2, five `1600x1000` frames, purple brand palette). The shipped build scripts are Style B; trust the `build_*.py` + script over the prompt file when they disagree.
```
