...
```bash
python3 videos/ai-training/3-prompting/build_excalidraw.py
python3 preview.py videos/ai-training/3-prompting/prompting-crispe-framework.excalidraw out.png [XMIN XMAX]
```
...
- **Folder:** the board lives at `videos/ai-training/3-prompting/`. `videos/claude/13-` is occupied by the ChatGPT→Claude video, and `videos/ai-training/` is the authoritative series (see CLAUDE.md).
